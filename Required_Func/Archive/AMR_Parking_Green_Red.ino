...
/*
Parking sketch goal:
1. Run the same stage-1 light alignment used in AMR_Final_V3.
2. Detect GREEN or RED and branch to the assigned parking side.
3. Rotate 90 degrees, approach the side wall, and do the same light assist used in V3.
4. Finish with ultrasonic parking in the assigned zone.
5. Keep retrying until parking succeeds.
*/
...
bool initialForwardUntilWallSeen() {
  ...
  Serial.println(F("[INITIAL] Start forward detection with dynamic 35/45 cm threshold"));

  while (true) {
    UltraPair pair = readUltrasonicPair();
    printUltraPair(F("INITIAL"), pair);

    float detectThresholdCm = INITIAL_FORWARD_DETECT_CM;
    if (pair.valid && floatAbs(pair.diffCm) < INITIAL_SKEW_BACKUP_DIFF_CM) {
      detectThresholdCm = TARGET_WALL_DIST_CM;
    }

    bool leftHit = pair.leftValid && pair.leftCm <= detectThresholdCm;
    bool rightHit = pair.rightValid && pair.rightCm <= detectThresholdCm;
    ...
    if (leftHit || rightHit) {
      ...
      Serial.print(F("[INITIAL] First wall detection reached at thresholdCm="));
      Serial.println(detectThresholdCm, 1);
      if (pair.valid && floatAbs(pair.diffCm) > INITIAL_SKEW_BACKUP_DIFF_CM) {
...
bool runAutoSequence() {
  Serial.println();
  Serial.println(F("========== PARKING START =========="));
  setStatus("AUTO", "Parking run");
  ...
  Serial.println(F("[FINAL] Parking sequence complete."));
  centerServo();
  stopMotors();
...
void printHelp() {
  Serial.println();
  Serial.println(F("=== AMR Parking Commands ==="));
  Serial.println(F("G : run parking auto sequence"));
...
void setup() {
  ...
  Serial.println(F("ELEC3848 green/red parking sketch"));
  Serial.println(F("Uses: light align -> color branch -> 90deg turn -> side-wall approach -> ultrasonic parking"));
  Serial.println(F("Keeps retrying until parking succeeds."));
