...
const float INITIAL_STEER_MIN_DIFF_CM = 10.0f;
const float INITIAL_STEER_MAX_DIFF_CM = 60.0f;
const float INITIAL_STEER_MIN_PWM_DIFF_RATIO = 0.05f;
const float INITIAL_STEER_MAX_PWM_DIFF_RATIO = 0.15f;
...
void driveForwardSteered(int pwm, const UltraPair &pair);
...
void driveForwardSteered(int pwm, const UltraPair &pair) {
  ...
  if (diffAbs < INITIAL_STEER_MIN_DIFF_CM) {
    driveForward(pwm);
    return;
  }

  float steerRatio = INITIAL_STEER_MIN_PWM_DIFF_RATIO;
  if (diffAbs >= INITIAL_STEER_MAX_DIFF_CM) {
    steerRatio = INITIAL_STEER_MAX_PWM_DIFF_RATIO;
  } else {
    float t = (diffAbs - INITIAL_STEER_MIN_DIFF_CM) / (INITIAL_STEER_MAX_DIFF_CM - INITIAL_STEER_MIN_DIFF_CM);
    steerRatio = INITIAL_STEER_MIN_PWM_DIFF_RATIO +
                 t * (INITIAL_STEER_MAX_PWM_DIFF_RATIO - INITIAL_STEER_MIN_PWM_DIFF_RATIO);
  }

  int sideDelta = (int)((float)pwm * steerRatio * 0.5f + 0.5f);
  int leftPwm = pwm;
  int rightPwm = pwm;

  if (pair.diffCm > 0.0f) {
    // Left is farther, so bias forward motion to turn right toward the closer wall on the right.
    leftPwm += sideDelta;
    rightPwm -= sideDelta;
  } else {
    // Right is farther, so bias forward motion to turn left toward the closer wall on the left.
    leftPwm -= sideDelta;
    rightPwm += sideDelta;
  }

  Serial.print(F("[INITIAL_STEER] diff="));
  Serial.print(pair.diffCm, 1);
  Serial.print(F(" steerRatio="));
  Serial.print(steerRatio * 100.0f, 1);
...
bool initialForwardUntilWallSeen() {
  ...
  while (true) {
    ...
    if ((pair.leftValid && pair.leftCm <= STOP_DIST_CM) || (pair.rightValid && pair.rightCm <= STOP_DIST_CM)) {
      ...
      driveForwardSteered(PWM_INITIAL_FORWARD, pair);
      continue;
    }
