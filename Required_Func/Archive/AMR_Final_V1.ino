...
const unsigned long COLOR_WATCH_SAMPLE_MS = 100UL;
const int COLOR_WATCH_CONFIRM_COUNT = 2;
const unsigned long COLOR_FINAL_SAMPLE_MS = 100UL;
const int COLOR_FINAL_CONFIRM_COUNT = 2;
...
bool recoverStage1AfterNoColor() {
  Serial.println(F("[COLOR] Retry path -> backup, approach to 25 cm, full scan, first strafe, then normal follow-up alignment."));
  ...
  if (!(abs(angleError) <= CENTER_TOL_DEG || plateauCentered)) {
    ...
    if (!runParallelStepShiftByDistance(strafeRightNow, angleError, STAGE1_LDR_WALL_DIST_CM, true, F("RETRY_PARALLEL_STEP"))) {
      if (stage1EarlyColorTrigger) {
        Serial.println(F("[COLOR] Early color trigger captured during retry first strafe."));
        return true;
      }
      return false;
    }
  ...
  return centerLightBySumPeakAtDistance(TARGET_WALL_DIST_CM, TARGET_DEPTH_TOL_CM, STOP_DIST_CM, true, STAGE1_LDR_WALL_DIST_CM, STAGE1_MIN_STRAFE_MS, STAGE1_MAX_STEP_SHIFT_MS, false);
}
...
bool handlePostStageColor(bool allowRetry) {
  ...
  if (!centerLightBySumPeakAtDistance(SECOND_STAGE_TARGET_WALL_DIST_CM, SECOND_STAGE_TARGET_DEPTH_TOL_CM, SECOND_STAGE_STOP_DIST_CM, true, SECOND_STAGE_LDR_WALL_DIST_CM, STAGE2_MIN_STRAFE_MS, STAGE2_MAX_STEP_SHIFT_MS, false)) return false;
...
bool centerLightBySumPeakAtDistance(float targetDistanceCm, float targetTolCm, float stopGuardCm, bool useDistanceModel, float sensorWallDistCm, unsigned long minStrafeMs, unsigned long maxStrafeMs, bool watchColorOnFirstStrafe) {
  ...
  while (true) {
    ...
    bool strafeRightNow = POSITIVE_ANGLE_MEANS_STRAFE_RIGHT ? (angleError > 0) : (angleError < 0);
    if (useDistanceModel) {
      bool watchColor = watchColorOnFirstStrafe && (pass == 1);
      if (!runParallelStepShiftByDistance(strafeRightNow, angleError, sensorWallDistCm, watchColor, F("PARALLEL_STEP"))) {
        if (stage1EarlyColorTrigger) {
          Serial.println(F("[CENTER] Early color trigger captured during first strafe."));
          return true;
        }
        return false;
      }
